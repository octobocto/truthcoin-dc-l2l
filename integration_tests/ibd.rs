...
use plain_bitassets_app_rpc_api::node::{
    PrivateRpcClient as _, RpcClient as _,
};
...
async fn initial_block_download_task(
    ...
    let (mut enforcer_post_setup, mut bitassets_nodes) =
    ...
    let expected_syncer_blocks = match syncer_start {
        ...
        SyncerStart::OwnChainWithDeposit => {
            ...
            bitassets_nodes
                ...
                .await?;
            let deposit_address =
                bitassets_nodes.syncer.get_deposit_address().await?;
    ...
    // Check that sender and syncer have all blocks, on the same tip
    {
        ...
        let sender_tip = bitassets_nodes
            .sender
            .rpc_client
            .get_best_sidechain_block_hash()
            .await?;
        let syncer_tip = bitassets_nodes
            .syncer
            .rpc_client
            .get_best_sidechain_block_hash()
            .await?;
