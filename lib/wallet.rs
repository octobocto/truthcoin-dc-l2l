...
mod test {
    ...
    use crate::{
        types::{
            Address, FilledOutput, GetBitcoinValue as _, OutPoint, Output,
        },
        wallet::{Error, Wallet},
    };
    ...
    fn test_get_receive_address() -> anyhow::Result<()> {
        ...
        let test_dir = std::env::temp_dir()
            .join(format!("bitassets_test_receive_{nanos}"));
        if test_dir.exists() {
    ...
    fn test_get_or_generate_last_address() -> anyhow::Result<()> {
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("bitassets_test_wallet_{nanos}"));
    ...
    fn funded_wallet(
        ...
        let nanos = std::time::SystemTime::now()
            ...
            .as_nanos();
        let test_dir =
            std::env::temp_dir().join(format!("bitassets_test_{name}_{nanos}"));
        if test_dir.exists() {
            let _unused = std::fs::remove_dir_all(&test_dir);
        }
        let wallet = Wallet::new(&test_dir)?;
