...
// First 4 bytes are the US-TTY (LSB Left) Baudot–Murray code for "BTAST".
// The least significant bits of the 4th byte encode the network
// identifier.
pub const fn magic_bytes(network: Network) -> MagicBytes {
    const PREFIX: [u8; 4] =
        [0b1100_1100, 0b0000_0110, 0b0101_1000, 0b0000_0000];
...
mod network_tests {
    ...
    const EXPECTED: [(Network, MagicBytes); 4] = [
        (Network::Regtest, [0xcc, 0x06, 0x58, 0x00]),
        (Network::Signet, [0xcc, 0x06, 0x58, 0x01]),
        (Network::Forknet, [0xcc, 0x06, 0x58, 0x02]),
        (Network::Betanet, [0xcc, 0x06, 0x58, 0x04]),
    ];
